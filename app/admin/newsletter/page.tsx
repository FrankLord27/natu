...
const TableCard = styled.div`
  background: white; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); overflow: hidden;
`;

const Table = styled.table`width: 100%; border-collapse: collapse;`;
const Th = styled.th`
  text-align: left; padding: 18px 25px; background: #f8f8f8; font-weight: 800;
  font-size: 0.85rem; text-transform: uppercase; color: #888; border-bottom: 2px solid #eee;
`;
const Td = styled.td`padding: 18px 25px; border-bottom: 1px solid #f5f5f5; font-size: 0.95rem;`;
...
const ActionIcon = styled.button<{ $color?: string }>`
  ...
  &:hover { background: #f5f5f5; color: ${p => p.$color || '#333'}; }
`;
...
export default function AdminNewsletter() {
  ...
  const fetchSubscribers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/newsletter?page=${page}&limit=15&search=${searchTerm}`);
  ...
  const exportCSV = () => {
    ...
    const rows = subscribers.map(s => [s.email, s.isActive ? 'Activo' : 'Inactivo', new Date(s.createdAt).toLocaleDateString()]);
  ...
  return (
    <Page>
      ...
      <SearchBar>
        ...
        <input placeholder="Buscar suscriptores..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
      </SearchBar>
      ...
      <TableCard>
        <Table>
          ...
          <tbody>
            {loading ? <tr><Td colSpan={4} style={{ textAlign: 'center', padding: 40 }}>Cargando...</Td></tr> :
              subscribers.length === 0 ? <tr><Td colSpan={4} style={{ textAlign: 'center', padding: 40 }}>Sin suscriptores aún.</Td></tr> :
                subscribers.map(s => (
                  <tr key={s.id}>
                    <Td style={{ fontWeight: 700 }}><Mail size={14} style={{ marginRight: 8, verticalAlign: 'middle' }} /> {s.email}</Td>
                    <Td><Status $active={s.isActive}>{s.isActive ? 'ACTIVO' : 'INACTIVO'}</Status></Td>
                    <Td>{new Date(s.createdAt).toLocaleDateString()}</Td>
                    <Td>
                      <div style={{ display: 'flex', gap: 5 }}>
                        <ActionIcon $color={s.isActive ? '#e67e22' : '#2e7d32'} onClick={() => toggleStatus(s)} title={s.isActive ? 'Desactivar' : 'Activar'}>
                          {s.isActive ? <UserX size={18} /> : <UserCheck size={18} />}
                        </ActionIcon>
                        <ActionIcon $color="#ff5252" onClick={() => handleDelete(s.id)}>
                          <Trash2 size={18} />
                        </ActionIcon>
                      </div>
                    </Td>
