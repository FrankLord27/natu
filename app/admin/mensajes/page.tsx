...
const MessageCard = styled(motion.div)<{ $responded: boolean }>`
  ...
  padding: 25px;
  border-radius: 16px;
  border-left: 5px solid ${(p) => (p.$responded ? "#7BB32E" : "#FFB800")};
  box-shadow: 0 4px 15px rgba(0, 0, 0, 0.03);
  display: grid;
...
const ActionBtn = styled.button<{ $primary?: boolean }>`
  ...
  &:hover {
    background: ${(p) => (p.$primary ? p.theme.colors.primary : "#ff5252")};
    color: white;
  }
`;
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/messages?page=${page}&limit=10&search=${searchTerm}`,
      );
  ...
  return (
    <Page>
      ...
      {loading ? (
        ...
        <MessagesList>
          {messages.length === 0 ? (
            <p style={{ textAlign: "center", color: "#999", padding: 50 }}>
              ...
              >
                ...
                <Actions>
                  <ActionBtn $primary onClick={() => toggleResponded(m)}>
                    <CheckCircle size={16} />{" "}
                    {m.responded ? "Marcar Pendiente" : "Marcar Respondido"}
                  </ActionBtn>
