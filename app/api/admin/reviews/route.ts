...
export async function PATCH(req: Request) {
  ...
  try {
    const { id, isApproved, adminComment, isFeaturedTestimonial, testimonialOrder } = await req.json();
    
    const updateData: any = {};
    if (isApproved !== undefined) updateData.isApproved = isApproved;
    if (adminComment !== undefined) updateData.adminComment = adminComment;
    if (isFeaturedTestimonial !== undefined) updateData.isFeaturedTestimonial = isFeaturedTestimonial;
    if (testimonialOrder !== undefined) updateData.testimonialOrder = testimonialOrder;
