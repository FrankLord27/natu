...
const Reveal = ({
  ...
  const mainControls = useSpring(isInView ? 1 : 0, {
    stiffness: 50,
    damping: 20,
  });
...
const HeroSection = styled.section`
  ...
  background: radial-gradient(
    circle at 50% 50%,
    ${(p) => p.theme.colors.backgroundWarm} 0%,
    ${(p) => p.theme.colors.primaryPale} 100%
  );
  position: relative;
  overflow: hidden;
  padding: 60px 24px;
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (catRes.success)
        setCategories((catRes.data as Category[]).slice(0, 4));
    };
    fetchData();

    fetch("/api/testimonials")
      .then((r) => r.json())
      .then((d) => setTestimonials(d.slice?.(0, 3) || []))
      .catch(() =>
        setTestimonials([
          {
            id: "1",
  ...
  return (
    <>
        ...
        <Section $bg="#fcfcfc">
          <Reveal width="100%">
            <SectionHeader>
              ...
              <p>
                Explora nuestras líneas de productos creadas especialmente para
      ...
      {videos.length > 0 && (
        <Section
          $bg={`linear-gradient(135deg, #f8faf5 0%, #f0f4ec 100%)`}
      ...
      <Section $bg="#fff">
        <Reveal width="100%">
          <SectionHeader>
            ...
            <p>
              Únete a miles de personas que ya transformaron su vida con
              NaturaJM.
            </p>
          </SectionHeader>
          <TestGrid>
