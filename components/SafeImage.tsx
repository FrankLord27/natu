...
const FallbackWrapper = styled.div<{ $width?: any, $height?: any, $fill?: boolean }>`
  width: ${props => props.$fill ? '100%' : (props.$width ? `${props.$width}px` : '100%')};
  height: ${props => props.$fill ? '100%' : (props.$height ? `${props.$height}px` : '100%')};
  background: #f8f8f8;
...
const SafeImage: React.FC<SafeImageProps> = (props) => {
  ...
  const isCloudinary = src.includes('res.cloudinary.com') || (!src.startsWith('http') && !src.startsWith('/') && src.length > 2);
  ...
  if (error) {
    return (
      <div style={{ position: 'relative', width: fill ? '100%' : width, height: fill ? '100%' : height, borderRadius: 'inherit', overflow: 'hidden' }}>
  ...
  return (
    <Image 
      ...
      onError={() => {
        console.warn(`SafeImage: next/image failed for ${src}. Falling back to <img> tag.`);
        setError(true);
      }}
    />
  );
