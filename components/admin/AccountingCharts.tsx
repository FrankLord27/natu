  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <AreaChart data={trends}>
        ...
        <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 600, fill: '#999' }} />
        ...
        />
        <Area type="monotone" dataKey="income" stroke="#7BB32E" strokeWidth={3} fillOpacity={1} fill="url(#colorIncome)" name="Ingresos" />
        <Area type="monotone" dataKey="expense" stroke="#f44336" strokeWidth={3} fill="transparent" name="Egresos" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
      ...
      <BarChart data={trends}>
        ...
        <Bar dataKey="profit" radius={[8, 8, 0, 0]} name="Beneficio">
          {trends.map((entry: any, index: number) => (
            <Cell key={`cell-${index}`} fill={entry.profit >= 0 ? '#7BB32E' : '#f44336'} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
