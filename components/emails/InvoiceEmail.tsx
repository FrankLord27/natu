...
export const InvoiceEmail: React.FC<InvoiceEmailProps> = ({
  ...
  <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '600px', margin: '0 auto', color: '#333' }}>
    ...
    <div style={{ padding: '40px 20px', backgroundColor: '#ffffff', border: '1px solid #e9ecef' }}>
      ...
      <div style={{ margin: '30px 0', border: '1px solid #eee', borderRadius: '8px', padding: '20px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            {items.map((item, index) => (
              <tr key={index} style={{ borderBottom: index < items.length - 1 ? '1px solid #f5f5f5' : 'none' }}>
                <td style={{ padding: '10px 0' }}>{item.name} <span style={{ color: '#888' }}>x{item.quantity}</span></td>
                <td style={{ padding: '10px 0', textAlign: 'right' }}>${(item.price * item.quantity).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ borderTop: '2px solid #eee' }}>
              ...
              <td style={{ padding: '15px 0', textAlign: 'right', fontWeight: 'bold', fontSize: '1.2em', color: '#7BB32E' }}>
                ${total.toFixed(2)}
              </td>
            </tr>
          </tfoot>
    ...
    <div style={{ padding: '20px', textAlign: 'center', backgroundColor: '#f8f9fa', color: '#999', fontSize: '12px' }}>
